...
public class Daemon implements Runnable {
	...
	// Note: The speaker thread relays the system audio output (a dedicated PulseAudio null sink, see
	// SpeakerThread.ensureOutputSink()) to the LumiCube speaker. It is enabled by default; the
	// FOUNDRY_SPEAKER_ENABLED environment variable can be set to "false" to disable it.
	private static final boolean SPEAKER_ENABLED = Boolean.parseBoolean(
		System.getenv().getOrDefault("FOUNDRY_SPEAKER_ENABLED", "true")
	);
	...
	public void run() {
		...
		try {
			...
			if (SPEAKER_ENABLED) {
				...
				logger.info("Speaker audio forwarding is disabled (set FOUNDRY_SPEAKER_ENABLED=true to enable it).");
			}
