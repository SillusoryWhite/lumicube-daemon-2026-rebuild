...
public class RedisLauncher {
	...
	public synchronized void start() throws IOException {
		...
		// Note: Redis requires the loadmodule .so file to be executable (it checks X_OK), and on some
		// file systems/tmpfs mounts Java's File.setExecutable(...) does not actually set the permission bit.
		// Fall back to an explicit chmod so the bundled RedisTimeSeries module can be loaded.
		try {
			new ProcessBuilder("chmod", "a+x", server.getAbsolutePath()).redirectErrorStream(true).start().waitFor();
			new ProcessBuilder("chmod", "a+x", plugin.getAbsolutePath()).redirectErrorStream(true).start().waitFor();
		} catch (InterruptedException interruptedException) {
			Thread.currentThread().interrupt();
			throw new IOException("Interrupted whilst applying executable permissions to Redis binaries.", interruptedException);
		}
