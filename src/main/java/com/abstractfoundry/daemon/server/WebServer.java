...
public class WebServer {
	...
	public WebServer(Node daemonNode, ExecutorService globalPool, Store store, ScriptExecutor scriptExecutor, int port, GenericSocketClientHandlerSupplier handlerSupplier) {
		...
		// Note: Register the Jackson JSON provider so the JAX-RS resources can serialise their
		// representation objects to/from application/json. Without this, GET requests that return
		// representation objects fail with "MessageBodyWriter not found for media type=application/json".
		jerseyConfig.register(org.glassfish.jersey.jackson.JacksonFeature.class);
